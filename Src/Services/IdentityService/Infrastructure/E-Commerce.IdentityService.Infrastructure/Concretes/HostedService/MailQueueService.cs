        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                try
                {
                    ...
                    //await Task.WhenAll(smtp.ConnectAsync(_mailOptions.Server, _mailOptions.Port, cancellationToken: stoppingToken),
                    //                   smtp.AuthenticateAsync(_mailOptions.SenderEmail, _mailOptions.Password, stoppingToken),
                    //                   smtp.SendAsync(email),
                    //                   smtp.DisconnectAsync(true, stoppingToken));
                }
                catch (Exception)
                {
                    throw new Exception("");
